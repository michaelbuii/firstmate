...
When a compiler returns a complete typed worker header containing the identity, schema-v2 manifest, `# Task`, and opening Task text, pass that exact output to `fm-brief.sh --compiled-header-file`; the scaffold records the header and its durable handoff marker, so never keep only its manifest, remove the stored header, or rebuild its Task from the two provenance subsections.
