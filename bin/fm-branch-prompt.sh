...
3. Handle with real tools: `bin/fm-crew-state.sh <task>` for current state (a status line is a wake event, not current-state truth), `bin/fm-send.sh` for a short steer, `bin/fm-control.sh <task> interrupt|exit|relaunch` for lifecycle, `bin/fm-pr-check.sh <task> <url>` when the task's ready status or `pr=` metadata names the PR's URL, `tasks-axi` for backlog moves.
   A completed scout is not report-only, but automatic cleanup needs a trusted terminal state: when `bin/fm-crew-state.sh <task>` says `state: done` and its regular metadata says `kind=scout`, read and summarize its report, then run ordinary `bin/fm-teardown.sh <task>` while you still hold the task lease; teardown is the sole completion-gate authority.
   That guarded teardown preserves the report while retiring the finished worker, isolated copy, endpoint, and volatile records; never substitute manual deletion, a forced teardown, or label-based endpoint cleanup.
   Never promote a status line into trusted completion. In particular, `unknown codex-unverified` means Codex completion needs reconciliation: retain the scout and report that unverified completion instead of cleaning it up.
   If teardown refuses, preserve everything, report the concrete reason and consequence, and leave the retry or captain-owned decision to a later event rather than claiming cleanup succeeded.
   Do not tear down a ship merely because its worker is done: PR and local-only work remain until landing is confirmed through their delivery path, and failed work remains for recovery.
   If a replay names an already-cleaned task whose metadata is absent, treat that absence as idempotent only for the replay; never infer authority to delete any path or endpoint.
