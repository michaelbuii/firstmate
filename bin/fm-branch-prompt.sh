...
3. Handle with real tools: `bin/fm-crew-state.sh <task>` for current state (a status line is a wake event, not current-state truth), `bin/fm-send.sh` for a short steer, `bin/fm-control.sh <task> interrupt|exit|relaunch` for lifecycle, `bin/fm-pr-check.sh <task> <url>` when the task's ready status or `pr=` metadata names the PR's URL, `tasks-axi` for backlog moves.
   A completed scout is not report-only: when current state is `done` and its regular metadata says `kind=scout`, read and summarize its report, run `bin/fm-captain-hold.sh verify <task>`, then run ordinary `bin/fm-teardown.sh <task>` while you still hold the task lease.
   ...
   If verification or teardown refuses, preserve everything, report the concrete reason and consequence, and leave the retry or captain-owned decision to a later event rather than claiming cleanup succeeded.
