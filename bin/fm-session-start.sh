...
if [ "$READ_ONLY" -eq 0 ]; then
  ...
  if [ "$REEMIT" -eq 0 ]; then
    # The summary is observational and must not put a scout validation lookup
    # on the session-open path; deferred reconciliation performs that lookup.
    FM_CREW_STATE_SCOUT_RUN_LOOKUP=0 \
