...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  Decision-bearing signal, stale, and heartbeat rows get the identical treatment: their producers prefix the queued payload with `needs-decision:`, `scopeForUnreadWake` excludes every marked row from what the branch may claim, and the dispatcher forces a marked row's own signal, stale, or heartbeat trigger to main by cross-referencing the trigger key against the excluded keys.
  Signal rows are marked when their newly classified status span contains a `needs-decision:` event, including a rejected keyed transition wrapped as `reconciliation-required`; stale rows are marked through the shared `fm_wake_append_stale` helper when the endpoint is needs-decision or captain-held or when a caller classifies an earlier decision in the actionable span; and the heartbeat backstop is marked when its scan finds an actionable needs-decision span.
  The immediate watcher wake reason remains the ordinary `signal:<files>`, `stale:`, or `heartbeat` shape, so harness-arm consumers keep their existing interface while the Pi trigger reaches the captain directly without the supervision-branch hop.
  ...
  The identical non-veto guarantee applies to a co-present decision-bearing row.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every non-check, non-decision-bearing row in the unread queue is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
...
Only a scan already flagged as possibly captain-relevant emits the bare `heartbeat` wake; `.pi/extensions/fm-primary-pi-watch.ts` flags that offer `heartbeat: true`, and the branch accepts it without a project only when every non-check, non-decision-bearing row observed in the unread-queue eligibility check is either heartbeat-kind or a resolvable task-local signal or stale event.

A heartbeat is never vetoed or ridden into main by a co-present check row or a separately keyed decision-bearing row.
Both classes are permanently main-owned: they are excluded from what the branch may claim and left queued for main, while their own triggering watcher cycle is forced to main so nothing starves by being left behind.
A heartbeat whose own queued row is decision-bearing is itself forced to main.
Deferring the fleet review to main merely because some unrelated merge poll, Relay mention, or decision row happened to be sitting unread put a routine review in the captain's chat for a reason that had nothing to do with the fleet, and that coupling is gone.
...
`tests/fm-watch-triage.test.sh` covers `bin/fm-watch.sh`'s side of the contract end to end: genuine and reconciliation-required needs-decision signal rows are marked while routine signals stay unmarked, captain-held stale rechecks are marked, and a heartbeat surfacing a buried needs-decision is marked.
`tests/fm-supervision-events.test.sh` covers the Herdr push-transition path marking a stale wake when its actionable status span contains a needs-decision.
